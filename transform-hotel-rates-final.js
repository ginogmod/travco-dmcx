...
function convertStarsToNumber(stars, city) {
    // For Wadi Rum hotels, keep "Regular" and "Deluxe" as is
    if (city === "Wadi Rum" && (stars === "Regular" || stars === "Deluxe")) {
        return stars;
    }
    
    // For all other cases, convert text to numbers
    switch (stars) {
        case 'Five': return 5;
        case 'Four': return 4;
        case 'Three': return 3;
...
newRatesData.forEach(entry => {
    ...
    // Convert stars to number (or keep as is for "Regular" and "Deluxe" in Wadi Rum)
    const stars = convertStarsToNumber(entry.Stars, entry.City);
...
existingRatesData.forEach(entry => {
    // Skip entries with text-based star ratings "Three", "Four", "Five"
    if (entry.Stars === "Three" || entry.Stars === "Four" || entry.Stars === "Five") {
        return;
    }
